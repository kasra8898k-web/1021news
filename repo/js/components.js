const Components = {
  ...
  newsCard(item, type = 'news') {
    ...
    return `
    <a href="#/${route}/${item.id}" class="block bg-white rounded-2xl overflow-hidden card-hover shadow-sm hover:shadow-lg transition-all">
      <div class="aspect-video bg-gradient-to-br from-gray-100 to-gray-200 relative overflow-hidden">
        ${item.image ?
          `<img src="${item.image}" alt="${Utils.escapeHtml(item.title)}" class="w-full h-full object-cover">` :
          `<div class="w-full h-full flex items-center justify-center">
            <svg class="w-12 h-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
          </div>`}
        <div class="absolute top-3 right-3 flex gap-2">
          ${item.isImportant ? '<span class="badge bg-red-500 text-white text-xs">مهم</span>' : ''}
          ${item.isBreaking ? '<span class="badge text-xs" style="background:#FACC15;color:#713F12">خبر فوری</span>' : ''}
        </div>
        <div class="absolute bottom-3 right-3 badge text-xs" style="background:${catColor.bg};color:${catColor.text};border:1px solid ${catColor.border}">${item.category}</div>
      </div>
      <div class="p-4">
        <h3 class="font-bold text-gray-900 mb-2 line-clamp-2 leading-7">${Utils.escapeHtml(item.title)}</h3>
        <p class="text-sm text-gray-500 mb-3 line-clamp-2">${Utils.escapeHtml(item.excerpt || Utils.truncate(item.content, 100))}</p>
        <div class="flex items-center justify-between text-xs text-gray-400">
          <span>${Utils.timeAgo(item.createdAt)}</span>
          <span class="text-red-500 hover:text-red-600 font-medium">ادامه مطلب →</span>
        </div>
      </div>
    </a>`;
  },
  ...
  commentSection(entityType, entityId) {
    ...
    <div class="mt-8 border-t pt-6">
      ...
      <div class="mb-6">
        <textarea id="commentText" class="input mb-2" placeholder="نظر خود را بنویسید..." rows="3"></textarea>
        <button onclick="Pages.submitComment('${entityType}', '${entityId}')" class="btn btn-red btn-sm">ارسال نظر</button>
        <p class="text-xs text-gray-400 mt-1">نظر شما پس از تأیید مدیر نمایش داده می‌شود.</p>
      ...
      <div class="space-y-4 stagger" id="commentsContainer">
        ${comments.length === 0 ? '<p class="text-sm text-gray-400 text-center py-4">هنوز نظری ثبت نشده است.</p>' :
          comments.map(c => {
            const cu = Store.getById('users', c.userId);
            return `
            <div class="flex gap-3 slide-up">
              <div class="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-bold text-sm flex-shrink-0">${cu ? cu.fullName.charAt(0) : '?'}</div>
              <div class="flex-1">
                ...
                <p class="text-sm text-gray-600">${Utils.escapeHtml(c.text)}</p>
              </div>
            </div>`;
          }).join('')}
      </div>
  ...
  emptyState(message) {
    ...
    <div class="text-center py-16">
      <svg class="w-16 h-16 mx-auto text-gray-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
